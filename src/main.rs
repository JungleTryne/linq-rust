...
fn main() {
    ...
    .into_linq_iter()
    .select(|x: String| -> Vec<String> { x.split_whitespace().map(str::to_string).collect() })
    .select(|x: Vec<String>| x.into_linq_iter())
    .flatten()
    .select(|x: String| (x, 1))
    .group_by(|x: &(String, u32)| x.0.clone())
    .select(|(key, values): (String, Vec<_>)| (key, values.len() as u32))
    .to_list();
